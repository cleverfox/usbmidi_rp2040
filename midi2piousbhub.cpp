...
static struct repeating_timer tmr1;
...
#endif
{
    ...
    for(int i=0;i<12;i++)
    {
        ...
        gpio_set_input_enabled(octave_pin[i], true);
        gpio_set_input_hysteresis_enabled(octave_pin[i], true);
        octave_mask |= (1 << octave_pin[i]);
        if(octave_mask_shift > octave_pin[i]){
            octave_mask_shift = octave_pin[i];
        }
    }
    ...
    default_out_port = &usbdev_midi_out_port;
...
static bool repeating_timer_callback(struct repeating_timer *t) {
  ...
  txbuf[1] = 0x3C; // Note number
  txbuf[2] = 0x7F; // Velocity
...
int main()
{
    ...
    add_repeating_timer_us(1000, repeating_timer_callback, (void *)&instance, &tmr1);
    core0_booting = false;
